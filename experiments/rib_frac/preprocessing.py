...
def pp_patient(inputs):
    ...
    with open(os.path.join(cf.pp_dir, 'meta_info_{}.pickle'.format(pid)), 'wb') as handle:
        ...
        meta_info_dict = {'pid': pid, 'class_target': None, 'spacing': img.GetSpacing(), 'fg_slices': fg_slices}
...
if __name__ == "__main__":
    ...
    gt_dicts = json.load(open(cf.gt_file))

    if not os.path.exists(cf.pp_dir):
        os.mkdir(cf.pp_dir)

    # pool = Pool(processes=12)
    # p1 = pool.map(pp_patient, gt_dictrs.items(), chunksize=1)
    # pool.close()
    # pool.join()
    for i in gt_dicts.items():
        pp_patient(i)
